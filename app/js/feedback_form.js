...
var feedbackForm = (function (){ //самовызывающаяся функция

	var init = function(){ // только это будет доступно извне
				console.log('иницилизация модуля форма обратной связи');
				_setUpListners();
		},

		_setUpListners = function() { //слежка за событиями
			$('#form-feedback').on('submit', _submitForm);// когда сабмит - вызываем метод сабмитформ
		},
		  	...
		  	if (resultObject) {  //проверка наличие переменной ajax
		  		resultObject.done(function(ans) {
		  			...
		  			if ( status === 'OK'){ //проверка ок - срабатывает этот код
		  				...
		  				form.find('.success-result').text(mes).show();// сообщение вставляем в блок и выводим
		  			} else{
