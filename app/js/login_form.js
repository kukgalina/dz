...
var loginForm = (function (){ //самовызывающаяся функция

	var init = function(){ // только это будет доступно извне
				console.log('иницилизация модуля форма авторизации');
				_setUpListners();
		},

		_setUpListners = function() { //слежка за событиями
			$('#login-form').on('submit', _submitForm);// когда сабмит - вызываем метод сабмитформ
		},
		
		_submitForm = function (ev) {
			console.log('Работа с формой авторизации');
